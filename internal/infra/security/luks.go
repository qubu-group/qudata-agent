...
func CreateVolume(config VolumeConfig) string {
	if os.Geteuid() != 0 {
		logger.LogWarn("LUKS: not running as root")
		return ""
	}
	...
	if config.MountPoint == "" {
		logger.LogError("LUKS: mount point not specified")
		return ""
	}
	if config.SizeMB <= 0 {
		config.SizeMB = 10240
	...
	if C.luks_open_volume(cDevice, cMapper, cMountPoint, (*C.char)(cKey2), C.size_t(len(keyBytes))) != 0 {
		cleanup()
		logger.LogError("LUKS: failed to open volume")
		return ""
	}
...
func DeleteVolume() {
	...
	if !mgr.active {
		logger.LogWarn("LUKS: no active volume to delete")
		return
	}
