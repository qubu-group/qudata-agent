...
type UnitValue = domain.UnitValue
type ConfigurationData = domain.ConfigurationData

func configurationSnapshot() domain.ConfigurationData {
	return domain.ConfigurationData{
		RAM:         getRAM(),
		Disk:        getDisk(),
		CPUName:     getCPUName(),
...
func getNetworkSpeed() float64 {
	...
	if err != nil {
		logger.LogWarn("Failed to read /sys/class/net")
		return 1.0
	}
...
func readInterfaceSpeedViaEthtool(iface string) float64 {
	...
	if speed <= 0 && err != nil {
		logger.LogWarn("ethtool %s failed: %v", iface, err)
	}
	return speed
}
