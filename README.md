...
- **Туннель (FRP)** создаётся сразу при создании агента: при вызове API `/init` бэкенд возвращает параметры FRP (сервер, subdomain, токен), агент поднимает frpc и пробрасывает свой HTTP-порт наружу. **Обращение к агенту извне идёт только по туннелю** (по subdomain); локально агент слушает на `127.0.0.1`.
