...

{
    ...
    if ! ldconfig -p | grep -q libnvidia-ml.so; then
        ...
        NVML_LIB=$(find /usr/lib /usr/local/lib /lib -name "libnvidia-ml.so*" 2>/dev/null | head -n1)
        if [ -n "$NVML_LIB" ]; then
            echo "Found: $NVML_LIB, creating symlink" >> "$LOG_FILE"
            ln -sf "$NVML_LIB" /usr/lib/x86_64-linux-gnu/libnvidia-ml.so.1
